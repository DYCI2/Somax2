...
class PhaseModulationScaleAction(AbstractScaleAction):
    ...
    def filter(self, candidates: Candidates) -> Candidates:
        ...
        if isinstance(candidates, ContinuousCandidates):
            candidates.scale(np.exp(self.selectivity * (np.cos(2 * np.pi * (self.time - candidates.times)) - 1)))
        else:
            raise FilterError(f"class {self.__class__.__name__} only supports candidates of type"
                              f"{ContinuousCandidates.__name__}")
...
class NextStateScaleAction(AbstractScaleAction):
    ...
    def filter(self, candidates: Candidates) -> Candidates:
        ...
        event_indices: np.ndarray = candidates.get_indices()
        is_matching: np.ndarray = event_indices == self._previous_output_index + 1
        candidates.scale(self.factor, is_matching)
        return candidates

    def feedback(self, candidate: Optional[Candidate], **kwargs) -> None:
        if candidate is not None:
            self._previous_output_index = candidate.event.index
        else:
...
#         self._history: Queue[Candidate] = Queue(max_length=100)
#
#     def scale(self, peaks: Peaks, time: float, corresponding_events: List[SomaxCorpusEvent],
#               corresponding_transforms: List[AbstractTransform], corpus: SomaxCorpus = None, **kwargs) -> Peaks:
...
#         self._history: Queue[Candidate] = Queue(max_length=self._history_len.value)
